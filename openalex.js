/* openalex.js — OpenAlex columns + dashboard fixes for NL Research CRM */
(function () {
  'use strict';
  ...
  var REMOVE_LABELS = ['Open Deals', 'Actions Due'];
  ...
  function fixDashboard() {
    document.querySelectorAll('.stat-card').forEach(function(card) {
      ...
      if (REMOVE_LABELS.indexOf(label) !== -1) { card.remove(); return; }
      /* Fix institutions icon */
      var box = card.querySelector('.stat-icon-box');
      if (box && !box.querySelector('svg') && (card.getAttribute('onclick')||'').includes('institutions')) {
        box.innerHTML = INST_SVG;
      }
    });
  }
  ...
  function addColumns(table) {
    ...
    tbody.querySelectorAll('tr.sub-row').forEach(function (row) {
      ...
      if (info) {
        ...
        evTd.innerHTML = '<span class="oa-badge">Education \u00b7 NL</span><br>'
