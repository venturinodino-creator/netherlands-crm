...
(function () {
  ...
  /* ── Institution SVG icon (building-library) ──────────────────────────── */
  var INST_ICON = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" width="24" height="24">'
  ...
  function fixDashboardIcons() {
    document.querySelectorAll('.stat-card').forEach(function(card) {
      ...
      if (!box) return;
      if (onclick.includes('institutions')) box.innerHTML = INST_ICON;
    });
  }

  /* ── OpenAlex data (hardcoded — no API calls needed) ─────────────────── */
  var DB = {
