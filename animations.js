(function () {
  ...
  function staggerCards(root) {
    const selectors = ['.ic','.news-card','.pipeline-card','.stat-card','.dash-widget'];
...
/* ── PENDING CONTACTS RENDERER ───────────────────────────────────────
   Runs only when discovery.js v3 is absent. Tracks nav section via
   wrapper so render() always knows which page is active.
──────────────────────────────────────────────────────────────────────── */
(function () {
  'use strict';
  ...
  function render() {
    ...
    var filtered = pending.filter(function(c){
      var inst = INST[c.instId]||{}; return active.indexOf(inst.type||'research')!==-1;
    });
    ...
    if(pending.length>0) html+='<button onclick="window._nlAcceptAll()" style="background:#6366f1;color:#fff;border:none;border-radius:8px;padding:8px 18px;cursor:pointer;font-weight:600">Accept All</button>';
    html += '</div>';
    ...
    if(filtered.length===0){
      html+='<div style="text-align:center;padding:48px;color:#64748b">'+(pending.length>0?'No contacts match the active filters.':'No pending contacts. Click &ldquo;Find contacts now&rdquo; to discover new ones.')+'</div>';
    } else {
      ...
      filtered.forEach(function(c){
        ...
        html+='<div style="flex:1"><div style="font-weight:700;color:var(--text1,#f1f5f9)">'+(c.first||'')+' '+(c.last||'')+'</div>';
        html+='<div style="color:#94a3b8;font-size:.85rem">'+(c.title||'')+'</div>';
        html+='<div style="margin-top:6px;display:flex;gap:8px;flex-wrap:wrap"><span style="background:'+tm.bg+';color:'+tm.color+';border-radius:99px;padding:2px 10px;font-size:.75rem;font-weight:700;border:1px solid '+tm.color+'">'+tm.label+'</span><span style="color:#cbd5e1;font-size:.85rem">'+inst.name+'</span></div></div>';
        html+='<div style="display:flex;gap:8px"><button onclick="window._nlAccept(''+c.id+'')" style="background:#10b981;color:#fff;border:none;border-radius:8px;padding:6px 14px;cursor:pointer;font-weight:600;font-size:.85rem">Accept</button>';
        html+='<button onclick="window._nlDiscard(''+c.id+'')" style="background:#ef4444;color:#fff;border:none;border-radius:8px;padding:6px 14px;cursor:pointer;font-weight:600;font-size:.85rem">Discard</button></div></div>';
      });
      html+='</div>';
    }
    html+='</div>';
  ...
  window._nlAccept=function(id){
    var p=getP(),c=getC(),idx=p.findIndex(function(x){return x.id===id;});
    if(idx===-1)return;
  ...
  function detectCurrentSection(){
    // Check active nav item text
    var active=document.querySelector('.nav-item.active,[class*="nav-item"][class*="active"]');
    if(active&&active.textContent.toLowerCase().includes('new contact'))return'pending';
    // Check page header text
    var header=document.querySelector('h1,h2,.page-title');
    if(header&&header.textContent.includes('New Contact'))return'pending';
    return'';
  }

  function init(){
    _curSection=detectCurrentSection();
    wrapNav();
  ...
  console.log('[NL CRM] animations.js fallback renderer v2 loaded');
})();
