...
(function () {
    ...
    return fetch(JSON_URL + '?t=' + Date.now())
      ...
      .then(function(remote) {
        ...
        remote.forEach(function(c) {
          ...
          pending.push({ id:id, instId:c.instId||'', first:(c.first||'').trim(), last:(c.last||'').trim(),
            title:(c.title||'').trim(), dept:(c.dept||'').trim(), email:el, phone:'', linkedin:'', research:'',
        ...
        if (added > 0) { saveP(pending); try { if (typeof loadPendingCount==='function') loadPendingCount(); } catch(_){} }
        return added;
  ...
  window._nlAccept = function(id) {
    ...
    if (idx===-1) return;
    var contact = p.splice(idx,1)[0], inst = INST[contact.instId]||{}, contacts = getC();
    contacts.push(Object.assign({}, contact, { institution: inst.name||contact.instId||'', addedAt: new Date().toISOString() }));
    saveC(contacts); saveP(p);
    try { if (typeof loadPendingCount==='function') loadPendingCount(); } catch(_){}
    render();
  };
  window._nlDiscard = function(id) { saveP(getP().filter(function(c){ return c.id!==id; })); render(); };
  window._nlAcceptAll = function() {
    var p = getP(); if (!p.length) return;
    var contacts = getC();
    ...
    p.forEach(function(c) {
      var el=(c.email||'').toLowerCase().trim(), nl=(c.first+' '+c.last).toLowerCase().trim();
      if (el&&emails.has(el)) return; if (names.has(nl)) return;
      var inst=INST[c.instId]||{};
      contacts.push(Object.assign({},c,{institution:inst.name||c.instId||'',addedAt:new Date().toISOString()}));
  ...
  window._nlToggleT = function(t) { var types=getT(), idx=types.indexOf(t); if (idx===-1) types.push(t); else if (types.length>1) types.splice(idx,1); saveT(types); render(); };
  window._nlSaveCfg = function() { var t=getT(); try { if (typeof toast==='function') toast('Scraper will target: '+t.join(', '),'ok'); } catch(_){} };

  function wrapNav() {
    if (!window.nav || window._nlNavWrapped) return;
    window._nlNavWrapped = true;
    var origNav = window.nav;
    window.nav = function(section) { origNav(section); if (section==='pending') setTimeout(render, 350); };
  }

  window.runContactDiscovery = function() {
    try { if (typeof toast==='function') toast('Checking for new contacts...','ok'); } catch(_){}
    syncFromFile().then(function(n) { try { if (typeof toast==='function') toast(n>0?n+' new contacts loaded':'No new contacts yet','ok'); } catch(_){} });
  };

  function init() {
    syncFromFile().then(function(n) { if (n>0) { console.log('[NL CRM] Auto-loaded',n,'contacts'); render(); } });
    wrapNav(); render();
  }

  if (window.nav) { init(); }
  else {
    var attempts=0, poll=setInterval(function() { if (window.nav||++attempts>30) { clearInterval(poll); init(); } }, 150);
  }
